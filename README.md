...
- **전체 보고 모드**: 선택 상태와 무관하게 전사~최하위 부서까지 모든 조직을
  ...
  "표 전체 복사" 버튼을 누르면 표 형식(HTML) 그대로 클립보드에 복사되어 엑셀/워드에
  Ctrl+V로 붙여넣으면 셀이 그대로 나뉘어 들어갑니다(오래된 브라우저에서 클립보드 API가
  막혀 있으면 자동으로 표 선택 + 복사 방식으로 대체됩니다). "전년비 포함" / "문항별
  점수(30개) 포함" 체크박스로 표에 담을 컬럼 범위를 조절할 수 있습니다.
